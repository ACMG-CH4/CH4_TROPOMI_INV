...
def use_AK_to_GC(filename,GC_startdate, GC_enddate, use_Sensi,xlim,ylim,):
    ...
    #xlim=[-102,-86.8];ylim=[13,30]
    sat_ind=np.where((TROPOMI['longitude']>xlim[0]) & (TROPOMI['longitude']<xlim[1]) & (TROPOMI['latitude']>ylim[0]) & (TROPOMI['latitude']<ylim[1]) & (TROPOMI['qa_value']>=0.5) & (TROPOMI['localtime']>=GC_startdate) & (TROPOMI['localtime']<=GC_enddate))
...
def nearest_loc(loc0,table,tolerance=0.5):
